...
static vx_status VX_CALLBACK  myUserTestTargetFunction(tivx_target_kernel_instance kernel, tivx_obj_desc_t *obj_desc[], uint16_t num_params, void *priv_arg)
{
    /* Parameter 0: input with supplementary data equal to {1, 2, 3, 4}
       ...
       Parameter 4: additional "output" with smaller supplementary data
       Parameter 5: additional "output" with different type than vximage
    */
    ...
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, tivxExtendSupplementaryDataObjDesc(obj_desc[3], supp, &test_data, sizeof(test_data) / 4));
    EXPECT_EQ_VX_STATUS(VX_ERROR_INVALID_TYPE, tivxExtendSupplementaryDataObjDesc(obj_desc[4], supp, &test_data, sizeof(test_data) / 4));
    supp = tivxGetSupplementaryDataObjDesc(obj_desc[5], NULL);
    ERROR_CHECK_VX_SUCCESS(NULL == supp, "tivxGetSupplementaryDataObjDesc fails when there is supplementary data");
    return VX_SUCCESS;
}

void testAddTargetKernelGeneral(vx_context context)
{
    /* section to remove */
    ...
    general_test_kernel = vxAddUserKernel(context, "myUserTestTargetKernel", MY_USER_TEST_TARGET_KERNEL, NULL, 6, myGeneralKernelValidator, NULL, NULL);
    ...
    ERROR_CHECK_VX_SUCCESS(vxAddParameterToKernel(general_test_kernel, 4, VX_OUTPUT, VX_TYPE_REFERENCE, VX_PARAMETER_STATE_REQUIRED), NULL);
    ERROR_CHECK_VX_SUCCESS(vxAddParameterToKernel(general_test_kernel, 5, VX_OUTPUT, VX_TYPE_REFERENCE, VX_PARAMETER_STATE_REQUIRED), NULL);
...
void testRemoveTargetKernelGeneral(void)
{
...
TEST (supplementary_data, testRefCount)
{
    ...
    ASSERT_VX_OBJECT(supp_data = (vx_reference)(vxGetSupplementaryUserDataObject(test_object, NULL, &status)), VX_TYPE_USER_DATA_OBJECT);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, status);
    ASSERT_VX_OBJECT(supp_data_copy = (vx_reference)(vxGetSupplementaryUserDataObject(test_object, NULL, &status)), VX_TYPE_USER_DATA_OBJECT);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, status);
...
void testOneType(vx_context context, vx_enum type)
{
    ...
    ASSERT_VX_OBJECT(supp = vxGetSupplementaryUserDataObject(ref, NULL, &status), VX_TYPE_USER_DATA_OBJECT);
    if (status != (vx_status)VX_SUCCESS)
    {
...
TEST(supplementary_data, testGetSetErrors)
{
    ...
    ASSERT_VX_OBJECT(supp = vxGetSupplementaryUserDataObject((vx_reference)image, "user_data_t", &status), VX_TYPE_USER_DATA_OBJECT);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, status);
...
void testParentAndChild(vx_reference parent, vx_reference child, const char * msg)
{
    ...
    vx_user_data_object parent_data, child_data;
    ASSERT_VX_OBJECT(parent_data= vxGetSupplementaryUserDataObject(parent, "user_data_t", &status), VX_TYPE_USER_DATA_OBJECT);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, status);
    ASSERT_VX_OBJECT(child_data = vxGetSupplementaryUserDataObject(child, "user_data_t", &status), VX_TYPE_USER_DATA_OBJECT);
...
{
    vx_user_data_object exemplar;
    vx_image imageFromROI, grandchildImageFromImage;
    vx_tensor tensorFromROI_0, tensorFromROI_1;
    vx_context context = context_->vx_context_;
    vx_status status = VX_SUCCESS;
    ...
    VX_CALL(vxReleaseTensor(&tensorFromROI_0));
    VX_CALL(vxReleaseTensor(&tensorFromROI_1));
}
...
TEST(supplementary_data, testExemplars)
{
    ...
    for (i = 0; (i < 3) && ((vx_status)VX_SUCCESS == status); ++i)
    {
        ...
        ASSERT_VX_OBJECT(supp = vxGetSupplementaryUserDataObject(ref, "user_data_t", &status), VX_TYPE_USER_DATA_OBJECT);
        user_data_t read_data = {0};
        EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxCopyUserDataObject(supp, 0, sizeof(read_data), &read_data, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
        if (read_data.numbers[3] != user_data.numbers[3])
        {
    ...
    vx_node node1, node2;
    ASSERT_VX_OBJECT(node1 = dumbCopyNode(graph, images[0], virt), VX_TYPE_NODE);
    ASSERT_VX_OBJECT(node2 = dumbCopyNode(graph, virt, images[2]), VX_TYPE_NODE);
    ...
    VX_CALL(vxReleaseUserDataObject(&exemplar));
    ASSERT_VX_OBJECT(exemplar = vxGetSupplementaryUserDataObject((vx_reference)(images[3]), NULL, &status), VX_TYPE_USER_DATA_OBJECT);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, status);
    ...
    VX_CALL(vxReleaseUserDataObject(&exemplar));
    ASSERT_VX_OBJECT(exemplar = vxGetSupplementaryUserDataObject((vx_reference)(images[2]), NULL, &status), VX_TYPE_USER_DATA_OBJECT);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, status);
    ...
    for (i = 0; (i < 3) && ((vx_status)VX_SUCCESS == status); ++i)
    {
        ...
        for (int j = 0; (j < 3) && ((vx_status)VX_SUCCESS == status); ++j)
        {
            ...
            ASSERT_VX_OBJECT(supp = vxGetSupplementaryUserDataObject((vx_reference)img, "user_data_t", &status), VX_TYPE_USER_DATA_OBJECT);
            user_data_t read_data = {0};
            EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxCopyUserDataObject(supp, 0, sizeof(read_data), &read_data, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
            if (read_data.numbers[3] != user_data.numbers[3])
            {
...
TEST(supplementary_data, testExtend)
{
    ...
    user_data_t user_data = {.numbers = {1, 2, 3, 4}};
    user_data1_t bad_user_data = {.numbers = {1, 2}};
    vx_graph graph;
    ASSERT_VX_OBJECT(graph = vxCreateGraph(context), VX_TYPE_GRAPH);
    vx_user_data_object exemplar, bad_exemplar;
    ASSERT_VX_OBJECT(exemplar = vxCreateUserDataObject(context, "user_data_t", sizeof(user_data_t), &user_data), VX_TYPE_USER_DATA_OBJECT);
    ASSERT_VX_OBJECT(bad_exemplar = vxCreateUserDataObject(context, "bad_user_data", sizeof(bad_user_data), &bad_user_data), VX_TYPE_USER_DATA_OBJECT);
    ...
    user_data1_t bad_new_data = {.numbers = {10, 11}};
    user_data_t read_data = {0};
    vx_status status = VX_SUCCESS;
    ...
    ERROR_EXPECT_STATUS(vxExtendSupplementaryUserDataObject((vx_reference)(image), bad_exemplar, &bad_new_data, sizeof(bad_user_data), sizeof(bad_user_data) / 2), VX_ERROR_INVALID_TYPE, "Correct error for dst_obj_desc->mem_size != src_obj_desc->mem_size");
    ...
    user_data_t empty = {0};
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxCopyUserDataObject(supp, 0, sizeof(empty), &empty, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST));
    vx_size valid_size = sizeof(user_data) / 4;
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxSetUserDataObjectAttribute(exemplar, VX_USER_DATA_OBJECT_VALID_SIZE, &valid_size, sizeof(valid_size)));
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxExtendSupplementaryUserDataObject((vx_reference)(subimage), exemplar, &new_data,  sizeof(user_data)/ 2,  3 * sizeof(user_data) / 4));
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxCopyUserDataObject(supp, 0, sizeof(user_data_t), &read_data, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
    ERROR_EXPECT_STATUS_BAD(read_data.numbers[0] == user_data.numbers[0] &&
    ...
    VX_CALL(vxReleaseUserDataObject(&exemplar));
    VX_CALL(vxReleaseUserDataObject(&bad_exemplar));
...
TEST(supplementary_data, testObjectDescriptors)
{
    ...
    user_data_t user_data = {.numbers = {1, 2, 3, 4}};
    user_data1_t test_data1 = {.numbers = {10, 20, 30}};

    vx_user_data_object exemplar;
    ASSERT_VX_OBJECT(exemplar = vxCreateUserDataObject(context, "user_data_t", sizeof(user_data_t), &user_data), VX_TYPE_USER_DATA_OBJECT);
    vx_user_data_object exemplar1;
    ASSERT_VX_OBJECT(exemplar1 = vxCreateUserDataObject(context, "user_data_t", sizeof(user_data1_t), &user_data), VX_TYPE_USER_DATA_OBJECT);    
    vx_image image;
    ASSERT_VX_OBJECT(image = vxCreateImage(context, 10, 10, VX_DF_IMAGE_U8), VX_TYPE_IMAGE);
    vx_image image1;
    ...
    ERROR_CHECK_VX_SUCCESS(vxSetSupplementaryUserDataObject((vx_reference)(output1), exemplar1), NULL);
    vx_pyramid output2;
    output2 = vxCreatePyramid(context, 2, VX_SCALE_PYRAMID_HALF, 16, 16, VX_DF_IMAGE_U8);
    EXPECT_EQ_VX_STATUS(VX_SUCCESS, vxSetSupplementaryUserDataObject((vx_reference)output2, exemplar));  
    vx_graph graph = vxCreateGraph(context);
    ...
    ERROR_CHECK_VX_SUCCESS(vxSetParameterByIndex(node, 4, (vx_reference)(output1)), NULL);
    ERROR_CHECK_VX_SUCCESS(vxSetParameterByIndex(node, 5, (vx_reference)(output2)), NULL);
    ...
    VX_CALL(vxReleasePyramid(&output2));
    VX_CALL(vxReleaseUserDataObject(&exemplar));
    VX_CALL(vxReleaseUserDataObject(&exemplar1));
    VX_CALL(vxReleaseNode(&node));
    VX_CALL(vxReleaseGraph(&graph));
...
TEST(supplementary_data, testReadOnlyObjects)
{
    ...
    vx_node node2;
    ASSERT_VX_OBJECT(node2 = dumbCopyNode(graph, virt, output2), VX_TYPE_NODE);
    ERROR_CHECK_VX_SUCCESS(vxProcessGraph(graph), NULL);
