...
{
    vx_int32 status = 0U;
    producer_msg_content_t* msg = (producer_msg_content_t*)message_buffer;
    msg->msg_type           = message_type;
    msg->consumer_id        = consumer->cons_base.consumer_id;
    msg->buffer_info.id     = buffer_id;
        ...
        ((consumer->cons_base.last_buffer_id == buffer_id) || (consumer->cons_base.last_buffer_dropped == 1)))
    {
        ...
        VX_PRINT(
            ...
            "\n");
        msg->buffer_info.last_buffer = 1U;
    }
...
static void* consumer_backchannel(void* arg);

static vx_int32
import_ref_from_producer(vx_consumer consumer, producer_msg_content_t* buff_desc_msg, vx_int32* fd, vx_uint32 numFd)
{
    vx_int32                   status           = VX_CONS_STATUS_FAILURE;
    ...
    if (buff_desc_msg->buffer_info.num_items > 0)
    {
        ...
        if (ref_export_handle->refDesc.meta.type == VX_TYPE_OBJECT_ARRAY)
        {
            ...
            {
                consumer->cons_base.refs[consumer->cons_base.num_refs] = objarray_ref;
                consumer->cons_base.num_refs++;
                consumer->cons_base.ipcMessageCount = 0; // Wrap the intermediate reference counter
                status                    = VX_CONS_STATUS_SUCCESS;
            }
        }
        else
        {
            ...
            memcpy(
                (void*)&consumer->cons_base.ipcMessageArray[consumer->cons_base.ipcMessageCount].refDesc,
    ...
    else if (buff_desc_msg->buffer_info.num_items == 0)
    {
        ...
        framework_status        = tivx_utils_import_ref_from_ipc_xfer(consumer->cons_base.context, ref_export_handle, &single_ref);
...
static vx_int32 handle_receive_message(vx_consumer consumer, vx_int32 socket_fd, vx_uint8* message_buffer)
{
    vx_int32  fd[VX_IPC_MAX_VX_PLANES];
    vx_uint32 num_fd = 0;
    vx_int32  status = 0;

    producer_msg_content_t* message_header = (producer_msg_content_t*)message_buffer;

    if (consumer->cons_base.last_buffer == 1U)
    {
    ...
    case VX_MSGTYPE_REF_BUF:
    {
        ...
        if (msg->buffer_info.last_buffer == 1)
        {
            ...
            status = pthread_create(&consumer->cons_base.backchannel_thread, NULL, consumer_backchannel, (void*)consumer);
            if (status != VX_CONS_STATUS_SUCCESS)
            {
            ...
            status = consumer->cons_base.create_graph_callback(consumer->cons_base.graph_obj, consumer->cons_base.refs, consumer->cons_base.num_refs);
            if (status != VX_CONS_STATUS_SUCCESS)
            {
            ...
            consumer->cons_base.ref_import_done = (vx_bool)vx_true_e;
        }
    }
    break;
    ...
    case VX_MSGTYPE_BUFID_CMD:
    {
        ...
        VX_PRINT(
            ...
            msg->buffer_info.id,
            msg->buffer_info.last_buffer);

        // check if metadata from IPC was received and apply it to consumer reference
        if (msg->buffer_info.metadata.is_valid == 1 && NULL != consumer->cons_base.receive_metadata_callback)
        {
            status = consumer->cons_base.receive_metadata_callback(
                consumer->cons_base.graph_obj, consumer->cons_base.refs[msg->buffer_info.id], &msg[1], msg->buffer_info.metadata.size);
        }
        ...
        VX_PRINT(
            ...
            msg->buffer_info.id,
            consumer->cons_base.refs[msg->buffer_info.id],
            msg->buffer_info.last_buffer,
            msg->buffer_info.last_frame_dropped);

        status = consumer->cons_base.enqueue_callback(consumer->cons_base.graph_obj, (vx_reference)consumer->cons_base.refs[msg->buffer_info.id]);

        if (msg->buffer_info.last_buffer == 1)
        {
            consumer->cons_base.last_buffer    = 1;
            consumer->cons_base.last_buffer_id = msg->buffer_info.id;
    ...
    else if (status == VX_MSGTYPE_CONSUMER_CREATE_DONE)
    {
        ...
        msg->buffer_info.last_buffer = 0U;
        status = send_buffer_release_message(consumer, message_buffer, msg->buffer_info.id, VX_MSGTYPE_CONSUMER_CREATE_DONE);
        if (status < 0)
        {
            consumer->cons_base.state = VX_CONS_STATE_FAILED;
        }
    }
    else if (status == VX_CONS_STATUS_REF_DROP)
    {
        // buffer was not enqueued, transmit the buffer back to the producer immediately
        consumer->cons_base.last_buffer_dropped = 1;
        producer_msg_content_t* msg       = (producer_msg_content_t*)message_buffer;
        msg->buffer_info.last_buffer = 0U;
        VX_PRINT(VX_ZONE_INFO, "CONSUMER: CONSUMER DROPS FRAME, BUFFER ID %d, %s.", msg->buffer_info.id, "\n");
        status = send_buffer_release_message(consumer, message_buffer, msg->buffer_info.id, VX_MSGTYPE_BUF_RELEASE);
        if (status < 0)
        {
            consumer->cons_base.state = VX_CONS_STATE_FAILED;
        }
...
static void* consumer_backchannel(void* arg)
{
    ...
    while (consumer->cons_base.state != VX_CONS_STATE_FLUSH)
    {
        ...
        {
            if (consumer->cons_base.state < VX_CONS_STATE_WAIT)
            {
...
static vx_gc_consumer_status_e start_sync_with_producer(vx_consumer consumer, char* access_point_name, producer_msg_content_t* msg)
{
    vx_int32 status    = 0;
    vx_int32 socket_fd = 0;
    ...
    while (1)
    {
        ...
        else if (consumer->cons_base.state == VX_CONS_STATE_FAILED)
        {
            return VX_CONS_STATUS_FAILURE;
        }
        else
        {
        ...
        if (consumer->cons_base.state == VX_CONS_STATE_FLUSH)
        {
            return VX_CONS_STATUS_FLUSHED;
        }
    }
    ...
    status = socket_write(socket_fd, (vx_uint8*)msg, NULL, 0);
    ...
    return VX_CONS_STATUS_SUCCESS;
}

void* consumerReceiverSockThread(void* arg)
{
    vx_uint8           message_buffer[SOCKET_MAX_MSG_SIZE];
    vx_consumer   consumer              = (vx_consumer)arg;
    producer_msg_content_t* msg                   = (producer_msg_content_t*)message_buffer;
    vx_uint32          done                  = 0;
    vx_int32           status                = 0;
    vx_int32           first_buffer_released = 0;
    ...
    while (!done)
    {
        ...
        {
            vx_gc_consumer_status_e gw_status = VX_CONS_STATUS_SUCCESS;
            // Wait for a socket connection with the producer
            gw_status = start_sync_with_producer(consumer, consumer->cons_base.access_point_name, msg);
            if (VX_CONS_STATUS_FAILURE == gw_status)
            {
                VX_PRINT(VX_ZONE_ERROR, "CONSUMER: cannot start sync with producer on master channel%s", "\n");
                consumer->cons_base.state = VX_CONS_STATE_FAILED;
            }
            else if (VX_CONS_STATUS_SUCCESS == gw_status)
            {
            ...
            else if (VX_CONS_STATUS_FLUSHED == gw_status)
            {
        ...
        case VX_CONS_STATE_RUN:
        {
            ...
            status = consumer->cons_base.dequeue_callback(consumer->cons_base.graph_obj, dequeued_refs, &num_dequeued_refs);
            ...
            {
                buffer_id = getBufferIdForConsumer(dequeued_refs[current_ref_num], consumer->cons_base.refs, consumer->cons_base.num_refs);
                VX_PRINT(
                    VX_ZONE_INFO,
        ...
        case VX_CONS_STATE_WAIT:
        {
            ...
            consumer->cons_base.last_buffer_transmitted = 1;
            consumer->cons_base.state                   = VX_CONS_STATE_FLUSH;
        }
        break;
        ...
        case VX_CONS_STATE_FAILED:
        {
            ...
            VX_PRINT(
                ...
                consumer->cons_base.num_failures + 1,
                "\n");
...
VX_API_ENTRY vx_status VX_API_CALL consumerStartSock(vx_consumer consumer)
{
    ...
    int thread_status = pthread_create(&consumer->cons_base.receiver_thread, NULL, consumerReceiverSockThread, (void*)consumer);
